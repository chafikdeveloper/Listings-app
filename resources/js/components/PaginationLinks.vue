        ...
        <div class="flex items-center rounded-md overflow-hidden shadow-lg">
            <div v-for="(link, index) in paginator.links" :key="index">
                <component
                    ...
                    :class="{'hover:bg-slate-300 dark:hover:bg-slate-500' : link.url, 'text-slate-300 dark:text-slate-500' : !link.url, 'font-bold text-indigo-500 dark:text-indigo-400' : link.active}"
                />
            </div>
        </div>
