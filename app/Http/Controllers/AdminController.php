...
class AdminController extends Controller
{
    public function index() {
        ...
        $users = User::with('listings')->filter(request(['search', 'user_role']))->paginate(2)->withQueryString();
    ...
    {
        $userListings = $user->listings()->filter(request(['search', 'disapproved']))->latest()->paginate(5)->withQueryString();
